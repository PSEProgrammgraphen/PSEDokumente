...
\vskip .13in
\hbox{{\bf  Interfaces}}
\entityintro{Constraint}{plugin.Constraint}{The most abstract interface for a constraint.}
...
\entityintro{LayoutRegister}{plugin.LayoutRegister}{Stores a collection of layouts for a specific graph.}
...
\entityintro{WorkspaceOption}{plugin.WorkspaceOption}{This is an option for a workspace to set and change settings on it.}
\vskip .1in
\vskip .1in
\subsection{\label{plugin.Constraint}\index{Constraint@\textit{ Constraint}}Interface Constraint}{
\vskip .1in 
The most abstract interface for a constraint. Can later be used to achieve compatibility between constraints of different domains and layouts.\vskip .1in 
...
\vskip .1in 
The importer interface is implemented when writting a class that can import files. This will enable Plugins that import specific files to be build. The main task of a class implementing this interface is to parse a FileInputStream into the Interface of an \texttt{\small IGraphModelBuilder}{\small 
\refdefined{graphmodel.IGraphModelBuilder}}. The \texttt{\small IGraphModelBuilder}{\small 
\refdefined{graphmodel.IGraphModelBuilder}} will then build the representation.\vskip .1in 
\subsubsection{Declaration}{
\begin{lstlisting}[frame=none]
...
This method parses an FileInputStream into an \texttt{\small IGraphModelBuilder}{\small 
\refdefined{graphmodel.IGraphModelBuilder}}. It has to ensure that all information is transfered to a correct graphmodelbuilder.
}
\item{
{\bf  Parameters}
  \begin{itemize}
...
\vskip .1in 
Stores a collection of layouts for a specific graph. This allows the client to select fitted layouts for every graph.\vskip .1in 
\subsubsection{Declaration}{
\begin{lstlisting}[frame=none]
...
{\bf  Parameters}
  \begin{itemize}
   \item{
\texttt{option} -- the layout option}
  \end{itemize}
}%end item
\end{itemize}
}%end item
...
\vskip .1in 
This is an option for a workspace to set and change settings on it.\vskip .1in 
\subsubsection{Declaration}{
\begin{lstlisting}[frame=none]
