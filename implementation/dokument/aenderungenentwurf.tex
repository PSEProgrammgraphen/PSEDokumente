...
\begin{longtable}{llp{0.8\linewidth}}
	...
	\change	{Replaced edgeKind (String) field from JoanaEdge with enum. Adapted interface of JoanaEdge accordingly.} 
			{All reason why using enum is better when possibility are known at compile-time.  (Lucas)}
	\change	{\textit{SerializedGraph} verschoben. Die \textit{GraphViewGraphFactory} wird nun serialisiert und nicht das Model.} 
			{Die \textit{GraphViewGraphFactory} besitzt mehr Information (z.B. die Größe der Knoten) über den Graphen als das Model. Diese Informationen sind vor allem für den \textit{SvgExporter} wichtig.}
